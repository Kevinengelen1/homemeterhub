...
def _dashboard_overview() -> str:
    return """
      ...
      <section class="summary-grid" aria-label="Period summary">
        ...
        <article class="summary-tile"><span>Solar export</span><strong id="summary-solar-export" aria-live="polite">—</strong><small>Sent to grid</small></article>
        <article class="summary-tile"><span>Solar now</span><strong id="summary-solar-power" aria-live="polite">—</strong><small>SolarEdge inverter</small></article>
      ...
      <section class="dashboard-charts">
        ...
        <figure class="chart-panel"><figcaption><h2>Solar production</h2><p>SolarEdge inverter output. Enable the optional SolarEdge collector to populate this chart.</p></figcaption><svg id="chart-solar" viewBox="0 0 900 250" role="img" aria-label="Solar production chart"></svg></figure>
      </section>
      ...
      (() => {
        const $ = id => document.getElementById(id);
        const metricCharts = [['chart-electricity','electricity_net_kwh'],['chart-gas','gas_m3'],['chart-water','watermeter_total_m3'],['chart-solar','solar_current_power_w']];
        const number = value => new Intl.NumberFormat('nl-NL',{maximumFractionDigits:3}).format(value);
        ...
        async function load() { const [start,end]=dates(), from=start.toISOString(), to=end.toISOString(), days=Number($('dashboard-range').value), interval=days<=2?'hour':days<=90?'day':'day'; $('dashboard-error').textContent=''; $('dashboard-period').textContent=`${start.toLocaleDateString()} – ${end.toLocaleDateString()}`; try { const summary=await fetch('/api/summary?'+query({from,to})).then(response=>response.json()); if(summary.error)throw new Error(summary.error); $('summary-net').textContent=format(summary.net_consumption_kwh,'kWh');$('summary-high').textContent=format(summary.high_tariff_kwh,'kWh');$('summary-low').textContent=format(summary.low_tariff_kwh,'kWh');$('summary-solar-export').textContent=format(summary.solar_export_kwh,'kWh');$('summary-solar-power').textContent=format(summary.solar_current_power_w,'W');$('summary-gas').textContent=format(summary.gas_m3,'m³');$('summary-water').textContent=format(summary.water_m3,'m³'); await Promise.all(metricCharts.map(async ([id,metric])=>{const aggregation=metric==='solar_current_power_w'?'avg':'delta';const response=await fetch('/api/history?'+query({metric,from,to,interval,aggregation}));const data=await response.json();if(!response.ok)throw new Error(data.error||'Unable to load chart');line($(id),data.points,data.unit);})); } catch(error) { $('dashboard-error').textContent=error.message; } }
